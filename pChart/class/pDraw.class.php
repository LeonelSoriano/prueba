 ...
 class pDraw
  {
   ...
   function my_leyenda($withd,$height,$valores){


    $cadena_mayor_tmp = $valores[0];

    for($i=0;$i < count($valores);$i++){

       if(strlen($valores[$i])>strlen($cadena_mayor_tmp)){
          $cadena_mayor_tmp = $valores[$i];
       }
    }


    $BoxArray = $this->getTextBox($withd,20,"../../pChart/fonts/verdana.ttf",7,60,$cadena_mayor_tmp . ' 00');
    ...
    $paddin = 10;
    $offset_adiccional = 6;
    $this->drawRoundedFilledRectangle($withd,$height,($Width[0]*2)+$offset_adiccional,$height+(10)*count($valores)+($spacio_linea*6)+$paddin,4,array("R"=>220,"G"=>220,"B"=>220,"Alpha"=>20,'BorderR' => 255,'BorderG' =>255,'BorderB' => 255));


    $Data = $this->DataSet->getData();//series->abcissas/picture

    $_r = array();
    $_b = array();
    $_g = array();

    foreach($Data['Extended']['Palette'] as $SerieName => $Serie){

      $R = $Serie["R"]; $G = $Serie["G"]; $B = $Serie["B"];
    ...
    for($i=0;$i < count($valores);$i++){
     ...
     $this->drawFilledRectangle($withd+5,$height+8+($spacio_linea*($i)),$withd+12,$height+((14)+$spacio_linea*($i)),array("R"=>$_r[$i],"G"=>$_g[$i],"B"=>$_b[$i],"Surrounding"=>20));

     $this->drawText($withd+15,($height+($spacio_linea*($i)))+10,$valores[(string)$i] . ' '. $i,array("R"=>0,"G"=>0,"B"=>0,"Align"=>TEXT_ALIGN_MIDDLELEFT,"FontSize"=>7));
