...
$ej = function($a){
    ...
    $codigo_departamento_sql = '';
    $codigo_departamento_sql_mno_gerencia = '';
    $codigo_departamento_sql_mno_gerencia_and = '';

    if($_POST['codigo_departamento'] == ''){
        $_POST['codigo_departamento'] = '*';
    }

    if($_POST['codigo_departamento'] != '*'  ){
            $codigo_departamento_post = $_POST['codigo_departamento'];
            $codigo_departamento_sql = ' AND cos_erogaciones.codigo_departamento = ' . $codigo_departamento_post;
            $codigo_departamento_sql_mno_gerencia = " WHERE mno_gerencia.codigo = $codigo_departamento_post";
            $codigo_departamento_sql_mno_gerencia_and = " AND mno_gerencia.codigo = $codigo_departamento_post";
    }


    $tipo_erogacion_apend = '';
    $tipo_informacion_apend = array();
    $index_actual = 0;
...
FROM
    ...
    WHERE cos_detalle_erogaciones.fecha LIKE '$anhio_post-$mes_post-%' $codigo_departamento_sql
ORDER BY cos_erogaciones.nombre";
    ...
    $a->_pdf->setMargenIzquierdo($paso*5);
    ...
    while($test = mysql_fetch_array($result)){
        ...
        $tipo_erogacion_apend = $nombre_unidad_erogacion;
...
WHERE cos_erogaciones.codigo_unidad_erogacion = $codigo_unidad_erogacion";
            ...
            if($cantidad != 0) {
                ...
                array_push($tipo_informacion_apend, array($gerencia,
                    $tipo_erogacion_apend,$costo/$cuenta*$cantidad));
...
//        array_push($datos_celda,  formatear_ve($total_depreciacion_departamento));
        ...
        $total_depreciacion_departamento = 0;
//        array_push($tipo_informacion_apend, array($nombre_departamento,
//            'Depreciación',$costo/$cuenta*$cantidad));
    ...
    for($i = 0;$i < count($tipo_informacion_apend);$i++){

        $datos_celda= array();
        $gerencia_actual = $tipo_informacion_apend[$i][0];
        $erogacion_actual = $tipo_informacion_apend[$i][1];
        $valor_actual = $tipo_informacion_apend[$i][2];

        if($gerencia_temporal != $gerencia_actual){

            if($primera_entrada){
                $primera_entrada = false;
            }else{

                array_push($datos_celda, utf8_multiplataforma('Sub Total'));
                array_push($datos_celda, utf8_multiplataforma(''));
                array_push($datos_celda,  formatear_ve($sub_total));
                $a->print_celda($tamanhios , $datos_celda,false);
            ...
            $a->_pdf->ln($separacion*2 );
            $a->print_sub_title($paso,utf8_multiplataforma($gerencia_actual));
            $a->_pdf->ln($separacion*1 );
            $gerencia_temporal = $gerencia_actual;

        }

        $datos_celda= array();
        array_push($datos_celda, utf8_multiplataforma($erogacion_actual));
        array_push($datos_celda, utf8_multiplataforma(''));
        array_push($datos_celda,  formatear_ve($valor_actual));
        $a->print_celda($tamanhios , $datos_celda,false);

        $sub_total += $valor_actual;


    }//gerencia - erogacion - valor
